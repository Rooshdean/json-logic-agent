...
def build_parser() -> argparse.ArgumentParser:
    ...
    scan = sub.add_parser("scan", help="Discover JSON files and interactively choose how to view them")
    scan.add_argument("path", nargs="?", default=".", help="Project directory; defaults to current directory")
    scan.add_argument("--json", action="store_true", help="Print machine-readable scan result; disables interactive mode")
    scan.add_argument("--no-interactive", action="store_true", help="Print the file list without opening the V4 picker")
    scan.add_argument("--max-bytes", type=int, default=1_000_000, help="Skip JSON files larger than this")
    scan.add_argument("--model", help="Override JSON_LOGIC_MODEL for translations launched from the picker")

    # Backward-compatible V1/V2/V3 form: jsonlogic file.json --to python
...
def _print_scan_result(result) -> None:
    print(f"JSON Logic Agent V4 — project scan\nRoot: {result.root}\n")
    if not result.files:
...
def _run_scan(args) -> None:
    ...
    # Interactive by default only for a real terminal. Pipes and CI retain deterministic output.
    if not args.no_interactive and sys.stdin.isatty() and sys.stdout.isatty():
        print(f"JSON Logic Agent V4\nFound {len(result.files)} JSON file(s) in {result.root}.\n")
        run_interactive_scan(
            result,
            lambda path, target: _run_explain(
                path, target, None, False, False, None, args.model
            ),
        )
        return
...
def _run_explain(input_path, target, out, show_model, show_trace, trace_out, model) -> None:
    ...
    if trace_out and result.trace:
        ...
        print(f"Wrote V4 trace to {trace_out}")
